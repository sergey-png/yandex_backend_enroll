...
from db.models import Item, Stats
        ...
        if found_item:
            ...
            if item_to_change.parentId:
                ...
                if found_item.parentId == item_to_change.parentId:
                    ...
                    update_item_parentId_without_changes(
                        session, item_to_change, found_item, data
                    )
                    ...
                    if found_item.parentId is not None:  # parentId is not None
                        ...
                        # something -> None
                        update_item_parentID_to_None(
                            session, found_item, item_to_change, data
                        )
                        # None -> something
                        update_item_parentId_from_None_to_something(
                            session, found_item, original_item_to_change, data
                        )
...
def get_one_element(item_id: str) -> Optional[ShopUnitView]:
    with create_session() as session:
        item = session.query(Item).filter(Item.id == item_id).first()
        if item:
            result_values = ShopUnitView(item)
            logger.info(
                'RESULT_VALUES CHILDREN ID = %s', result_values.children
            )
...
def delete_item(session: Any, item_to_delete: Item) -> bool:
    if item_to_delete.parentId is None:
        ...
        update_item_parentId_to_None_after_deleting(
            session, item_to_delete, item_to_delete
        )
...
def update_statistics(session: Any, item_to_update_id: str) -> bool:
    session.commit()
    item_to_update = (
        session.query(Item).filter(Item.id == item_to_update_id).first()
    )
    if item_to_update:
        items = (
            session.query(Stats)
            .filter(
                Stats.id == item_to_update_id,
                Stats.date != item_to_update.date,
            )
            .all()
        )
        if items:
            for item in items:
                session.query(Stats).filter(Stats.uuid == item.uuid).update(
...
def update_item_parentId_without_changes(
    session: Any, item_to_change: Item, found_item: Item, data: dict[str, Any]
) -> bool:
    new_data_for_others: dict[str, Any] = defaultdict(dict[str, Any])
    ...
    if item_to_change.type == 'OFFER':
        ...
        while True:
            ...
            if parent_item:
                if (
                    ...
                    parent_item.price = (
                        parent_item.all_price // parent_item.count_items
                        if parent_item.count_items
                        else None
                    )
        ...
        item_to_change.price = (
            item_to_change.all_price // item_to_change.count_items
            if item_to_change.count_items
            else None
        )
        ...
        while True:
            ...
            if parent_item:
                if (
                    ...
                    session.query(Item).filter(
                        Item.id == parent_item.id
                    ).update(dict(date=parent_item.date))
                    update_statistics(session, parent_item.id)
                item_to_change = parent_item
            else:
                break
...
# when found_item change parentId: None -> something
def update_item_parentId_from_None_to_something(
    session: Any, found_item: Item, item_to_change: Item, data: dict[str, Any]
) -> bool:
    session.query(Item).filter(Item.id == item_to_change.id).update(data)
    ...
    if item_to_change.type == 'OFFER':
        ...
        while True:
            parent_item = (
                session.query(Item)
                .filter(Item.id == found_item.parentId)
                .first()
            )
            if parent_item:
                if (
        ...
        item_to_change.price = (
            found_item.all_price // found_item.count_items
            if found_item.count_items
            else None
        )
        session.query(Item).filter(Item.id == item_to_change.id).update(
            dict(
...
def update_item_parentID_to_None(
    session: Any, found_item: Item, item_to_change: Item, data: dict[str, Any]
) -> bool:
    ...
    if item_to_change.type == 'OFFER':
        ...
        if data['price'] is None:
            data['price'] = (
                found_item_original.all_price
                // found_item_original.count_items
                if found_item_original.count_items
                else None
            )
        while True:
            parent_item = (
...
def update_item_parentId_to_None_after_deleting(
    session: Any, found_item: Item, item_to_change: Item
) -> bool:
    found_item_original = found_item.copy()
    if item_to_change.type == 'OFFER':
        while True:
...
def add_element(session: Any, item_to_add: Item, data: dict[str, Any]) -> bool:
    logger.info('*START*')
    logger.info(
        'Item with id "%s" does not exist. Creating new one...', item_to_add.id
    )
    # check if item_to_change with parentId
    if item_to_add.parentId:
        ...
        if (
            not session.query(Item)
            .filter(Item.id == item_to_add.parentId)
            .first()
        ):
            return False
...
def update_all_parents_1(
    session: Any, item_to_add: Item, data: dict[str, Any]
) -> bool:
    while True:
        parent_item = (
        ...
        if parent_item:
            if parent_item.type == 'CATEGORY' and item_to_add.type == 'OFFER':
                ...
                logger.info(
                    'Parent item is OFFER as well as item_to_add, ERROR'
                )
                return False
            elif (
                parent_item.type == 'OFFER' and item_to_add.type == 'CATEGORY'
            ):
                logger.info(
                    'Parent item is OFFER, but item_to_add is CATEGORY, ERROR'
                )
                return False
            elif (
                parent_item.type == 'CATEGORY'
                and item_to_add.type == 'CATEGORY'
            ):
...
def get_sales_from(item_date: datetime) -> list[dict[str, Any]]:
    with create_session() as session:
        # date - 24 Hours
        ...
        logger.info(
            'Got %s sales with items = %s', len(result_list), result_list
        )
        return result_list


def get_statistic_from(
    item_id: str, dateStart: datetime, dateEnd: datetime
) -> Optional[list[dict[str, Any]]]:
    with create_session() as session:
        if not session.query(Item).filter(Item.id == item_id).first():
            return None
        dateStart2 = dateStart.strftime('%Y-%m-%dT%H:%M:%S')
        dateEnd2 = dateEnd.strftime('%Y-%m-%dT%H:%M:%S')
        logger.info('Getting statistic from %s', dateStart2)
        logger.info('Getting statistic to %s', dateEnd2)
        items = (
            session.query(Stats)
            .filter(
                Stats.id == item_id,
                Stats.date >= dateStart2,
                Stats.date < dateEnd2,
            )
            .all()
        )
        ...
        logger.info(
            'Got %s statistic with items = %s', len(result_list), result_list
        )
        return result_list
