...
async def test_get_statistic_1(client):
    ...
    response = await client.get(f'node/{item_id}/statistic?dateStart=2022-02-01T00%3A00%3A00.000Z'
...
async def test_get_statistic_with_wrong_dataStart(client):
    item_id = "069cb8d7-bbdd-47d3-ad8f-82ef4c269df1"
    response = await client.get(f'node/{item_id}/statistic?dateStart=2022-??-01T00%3A00%3A00.000Z'
                                f'&dateEnd=2022-02-04T00%3A00%3A00.000Z')
...
async def test_delete_all(client):
    response = await client.delete('/delete/069cb8d7-bbdd-47d3-ad8f-82ef4c269df1')
    assert response.status_code == 200
